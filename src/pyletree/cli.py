...
def parse_cmd_line_arguments() -> argparse.Namespace:
    """Parse and validate command-line arguments for pyletree."""
    ...
    parser.add_argument(
        ...
        metavar="DIR_OR_FILE",
        help=(
            "respect .gitignore rules from given paths "
            "(defaults to current directory if omitted but flag is present)"
        ),
    )
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help=(
            "ignore .git folder and use .gitignore from given directories "
            "or directories containing .git (defaults to current directory "
            "if omitted but flag is present)"
        ),
    )
        ...
        if args.git:  # If specific paths provided
            for path_str in args.git:
                ...
                if not git_dir.exists() or not git_dir.is_dir():
                    parser.error(
                        f"--git path must be a .git directory or contain "
            ...
            for path_str in args.gitignore:
                ...
                if path.is_dir():
                    ...
                    if not gitignore_file.exists():
                        parser.error(
                            f"--gitignore directory must contain a .gitignore "
                            f"file: {path_str}"
                        )
